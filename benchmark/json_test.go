...
func BenchmarkFailoverCluster(b *testing.B) {
	b.ReportAllocs()
	b.ResetTimer()
	var data MediumPayload
	for i := 0; i < b.N; i++ {
		json.Unmarshal(mediumFixture, &data)
		time.Sleep(time.Millisecond*221)
	}
}

func BenchmarkSwitchMasterSlave(b *testing.B) {
	var data MediumPayload
	json.Unmarshal(mediumFixture, &data)
	b.ReportAllocs()
	b.ResetTimer()
...
func BenchmarkAddCluster(b *testing.B) {
	...
	for i := 0; i < b.N; i++ {
		...
		time.Sleep(time.Millisecond*265)
	}
}

func BenchmarkRemoveCluster(b *testing.B) {
	var data MediumPayload
	jsoniter.Unmarshal(mediumFixture, &data)
	b.ReportAllocs()
	for i := 0; i < b.N; i++ {
...
func BenchmarkGetCluster(b *testing.B){
	b.ReportAllocs()
	a := []byte(`{"doc_count_error_upper_bound":213,"sum_other_doc_count":164424,"buckets":[{"key":9999999,"doc_count":594},{"key":1009723,"doc_count":477},{"key":1002791,"doc_count":396},{"key":1004937,"doc_count":344},{"key":1023365,"doc_count":332},{"key":1002584,"doc_count":320},{"key":1029566,"doc_count":300},{"key":1029547,"doc_count":274},{"key":1031539,"doc_count":258},{"key":1000868,"doc_count":257}]}`)
	for i := 0;i < b.N; i++{
		jsonparser.Get(a, "buckets")
...
func BenchmarkAddNode(b *testing.B) {
	...
	for i := 0; i < b.N; i++ {
		...
		time.Sleep(time.Millisecond*223)
	}
}
